...
class TempestObservations(TempestBase):
    ...
    def obs_station(
        ...
        This endpoint returns a collection of observation arrays for the given
        station. You may optionally constrain by a start and/or end Unix epoch
        (seconds) and control aggregation with the ``bucket`` parameter.
        ...
        start_time : int | None, optional
            Unix epoch seconds (UTC) for the beginning of the time range. If
            provided must be > 0 and (if ``end_time`` given) <= ``end_time``.
        end_time : int | None, optional
            Unix epoch seconds (UTC) for the end of the time range. If
            provided must be > 0 and (if ``start_time`` given) >=
            ``start_time``.
        bucket : Bucket | int, optional
            Aggregation bucket size (minutes) accepted by the API. May be
            either a ``Bucket`` enum member or its underlying integer value.
            Defaults to 1 (no aggregation). Allowed values: 1, 5, 30, 180.
        obs_fields : str, optional
            Comma separated list of observation field names to limit the
        ...
        units_temp : UnitsTemp | str, optional
            Temperature units (allowed: 'c', 'f').
            Defaults to 'c'.
        units_wind : UnitsWind | str, optional
            Wind units (allowed: 'mph', 'kph', 'kts', 'mps', 'bft',
            'lfm').
            Defaults to 'mps'.
        units_pressure : UnitsPressure | str, optional
            Pressure units (allowed: 'mb', 'inhg', 'mmhg', 'hpa').
            Defaults to 'mb'.
        units_precip : UnitsPrecip | str, optional
            Precipitation units (allowed: 'mm', 'cm', 'in').
            Defaults to 'mm'.
        units_distance : UnitsDistance | str, optional
            Distance units (allowed: 'km', 'mi').
            Defaults to 'km'.

        Returns
        -------
        StationObservation
            Parsed station observation model.

        Raises
        ------
        ValueError
            If any validation fails (invalid station_id, time range, bucket,
            units or obs_fields).
        """
        ...
        validate_bucket = self._validate_enum_param("bucket", bucket_for_validate, Bucket)
        ...
        validated_precip = self._validate_enum_param("units_precip", units_precip, UnitsPrecip)
        validated_distance = self._validate_enum_param(
            "units_distance", units_distance, UnitsDistance
        )

        # Basic obs_fields validation (ensure no empty tokens & simple charset)
        if obs_fields:
    ...
    def obs_device(
        ...
        ----------
        device_id : int
            Unique device identifier. Must be positive.
        day_offset : int, optional
            Day offset relative to today for which to retrieve observations.
            API typically supports a small negative/positive window (e.g. 0 for
            today, 1 for yesterday).
        time_start : int | None, optional
            Unix epoch seconds (UTC) lower bound inside the chosen day (if
            provided must be > 0). Must be <= ``time_end`` if both provided.
        time_end : int | None, optional
            Unix epoch seconds (UTC) upper bound (if provided must be > 0).
        format : Format | str, optional
            Optional response format (e.g. "json" or "csv") if supported. Case
        ...
        -------
        DeviceObservation
            Parsed device observation model.
        ...
        # if day_offset < -365 or day_offset > 365:
        #     raise ValueError("day_offset out of allowed range (-365..365).")
    ...
    def obs_station_latest(
        ...
        ----------
        station_id : int
            Unique station identifier. Must be positive.

        Returns
        -------
        StationObservationLatest
            Latest station observation model instance.
        ...
        return self._get(f"observations/station/{station_id}")
