...
class DummyTransport(Transport):
    """Synchronous dummy transport that captures the last request and returns a canned response."""

    def __init__(self, content: dict | None = None, status_code: int = 200):
...
def make_client_with(content: dict | None = None) -> tuple[Tempest, DummyTransport]:
...
def test_station_invalid_id_raises():
...
def test_obs_station_success_defaults():
...
def test_obs_station_invalid_time_range_raises():
...
def test_obs_device_invalid_format_raises():
    client, _ = make_client_with({})
    with pytest.raises(ValueError):
...
def test_obs_station_latest_success():
    content = {"station_id": 1, "obs": []}
    client, tr = make_client_with(content)
...
def test_better_forecast_success():
...
def test_better_forecast_invalid_id_raises():
    client, _ = make_client_with({})
    with pytest.raises(ValueError):
        client.forecast(0)
...
def test_stats_invalid_id_raises():
    client, _ = make_client_with({})
    with pytest.raises(ValueError):
        client.stats(0)
